...
*/
//when number of cigars >= 40 && <= 60 then it isSuccessful
// when it isWeekend, isSuccessful = false; 
...
*/
function filterEvens(numbersToFilter){
...
// 	let oldArray = []; 
// 	let newArray = oldArray.filter((number) => {
// 		return number % 2 === 0;
// 	});
// 	return newArray
// }
...
10. **filterBigNumbers** Write a function that filters numbers greater than or equal to 100.
	...
	filterBigNumbers([]) → []
*/
function filterBigNumbers(numbersToFilter){
	return numbersToFilter.filter(
		(value) => value >= 100
	);
}
...
*/
function filterMultiplesOfX(numbersToFilter, x){
	return numbersToFilter.filter(
		(value) => value % x === 0
	);
...
//  filterMultiplesOfXArray = arr.filter(function(val))
// return (val % x) === 0; 
/*
...
function createObject(){
	...
	mySelf.firstName = 'Ratnesh';
	mySelf.lastName = 'Bhattacharya';
	mySelf.age = 38;
	return mySelf;
}
