...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let numberNodes = document.querySelectorAll(".number");
  numberNodes.forEach((elm) => {
    elm.addEventListener("click", clickNumber);
  });

  // let numberNodes1 = document.getElementsByClassName("number");
  // for (let i = 0; i < numberNodes1.length; i++) {
  //   numberNodes1[i].addEventListener("click", clickNumber);
  // }
  ...
  pointButton.addEventListener("click", clickNumber);
  ...
  operatorButtons.forEach((elm) => {
    elm.addEventListener("click", clickOperator);
  });

  // add click event listener for the equal sign
  // should call the function performOperation
  let equalOperator = document.querySelector(".equal-sign");
  equalOperator.addEventListener("click", performOperation);
});
