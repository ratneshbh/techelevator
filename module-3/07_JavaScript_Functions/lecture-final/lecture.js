...
function scopeTest() {
...

  {
    ...
    //change to var for out of scope access
...
function sumAllNumbers(numbersToSum) {
  ...
  return numbersToSum.reduce(
    // (acc, val) => acc + val  
      (acc, val) =>
        {
          console.log(`accumulator is ${acc} the current value is ${val}`);
          return acc + val;  
        } , 0
  );
}
...
const filterer = (value) => value % 3 === 0;

function allDivisibleByThreeNonAnon(numbersToFilter) {
  
  return numbersToFilter.filter(filterer);
