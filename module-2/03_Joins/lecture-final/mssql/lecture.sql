...
SELECT film.title, (actor.first_name + ' ' + actor.last_name) AS actorname
FROM film
...
-- (There aren't any great examples of left joins in the "dvdstore" database, 
-- so the following queries are for the "world" database)

-- A Left join, selects all records from the "left" table and matches them with records from the
-- "right" table if a matching record exists.
